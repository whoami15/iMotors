...
class PayPal
{
     ...
     * @param $loan
     */
    public function getCancelUrl($loan)
    {
        return route('paypal.checkout.cancelled', $loan->id);
    }

    /**
     * @param $loan
     */
    public function getReturnUrl($loan)
    {
        return route('paypal.checkout.completed', $loan->id);
    }

    /**
     * @param $loan
     */
    public function getNotifyUrl($loan)
    {
        $env = config('services.paypal.sandbox') ? "sandbox" : "live";

        return route('webhook.paypal.ipn', [$loan->id, $env]);
    }
}
